...
public class Inv3st_Plan {
    ...
    public static Inv3st_Plan input(String input) {
        ...
        synchronized (allowingPrfts1) {
            ...
            while (scanner.hasNext()) {
                ...
                String bestPrft = getBestPrft(amount, mList);
                
                allowingPrfts1.add(bestPrft);
            }
            
            
            String output1 = setOutput(allowingPrfts1);
            
            return new Inv3st_Plan(output1.toString());
        }
    }
    
    private static String getBestPrft(int amount, List<Integer> mList){
    ...
    private static String setOutput(List<String> allowingPrfts1){
    	String output = new String(new byte[]{});
    	for (int i = 0; i < allowingPrfts1.size(); i++) {
            if (i > 0) {
                output += "\n";
            }
            output += "Case" + format(" #%d: ", i + 1) + allowingPrfts1.get(i);
        }
